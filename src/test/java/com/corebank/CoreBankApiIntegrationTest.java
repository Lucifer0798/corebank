...
/**
 * Walks the full retail journey against a real Spring context and a real database:
 * log in, onboard a customer, verify them, open accounts, move money, and read the statement.
 *
 ...
 * steps: the idempotency claim commits in its own transaction, so a test-managed rollback
 * would hide the very behaviour being checked.
 */
@SpringBootTest
...
class CoreBankApiIntegrationTest {
    ...
    @Test
    @Order(1)
    @DisplayName("the bootstrap administrator can log in and mint a token")
    void adminCanLogIn() throws Exception {
        String body = mockMvc.perform(post("/api/v1/auth/login").contentType(JSON).content("""
                        {"username":"admin","password":"BootstrapTest#2025"}"""))
    ...
    @Test
    @Order(2)
    @DisplayName("a wrong password is rejected without revealing which half was wrong")
    void wrongPasswordIsRejected() throws Exception {
        mockMvc.perform(post("/api/v1/auth/login").contentType(JSON).content("""
    ...
    @Test
    @Order(4)
    @DisplayName("the administrator creates a teller login")
    void adminCreatesTeller() throws Exception {
        mockMvc.perform(post("/api/v1/auth/users").header("Authorization", bearer(adminToken))
                        .contentType(JSON).content("""
                        {"username":"teller-it","password":"TellerPass#2025","fullName":"Test Teller","roles":["TELLER"]}"""))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.roles[0]").value("TELLER"));

        String body = mockMvc.perform(post("/api/v1/auth/login").contentType(JSON).content("""
                        {"username":"teller-it","password":"TellerPass#2025"}"""))
                .andExpect(status().isOk())
                .andReturn().getResponse().getContentAsString();
        tellerToken = JsonPath.read(body, "$.accessToken");
    }
    ...
    void tellerOnboardsCustomer() throws Exception {
        String body = mockMvc.perform(post("/api/v1/customers").header("Authorization", bearer(tellerToken))
                        .contentType(JSON).content("""
    ...
    void unverifiedCustomerCannotOpenAccounts() throws Exception {
        mockMvc.perform(post("/api/v1/accounts").header("Authorization", bearer(tellerToken))
                        .contentType(JSON).content("""
    ...
    void kycRequiresAdmin() throws Exception {
        mockMvc.perform(patch("/api/v1/customers/{id}/kyc", customerId)
    ...
    void accountsAreOpened() throws Exception {
        String savings = mockMvc.perform(post("/api/v1/accounts").header("Authorization", bearer(tellerToken))
                        .contentType(JSON).content("""
                        {"customerId":"%s","accountType":"SAVINGS","currency":"INR"}""".formatted(customerId)))
                .andExpect(status().isCreated())
        ...
        String current = mockMvc.perform(post("/api/v1/accounts").header("Authorization", bearer(tellerToken))
                        .contentType(JSON).content("""
    ...
    void depositPostsBothLegs() throws Exception {
        mockMvc.perform(post("/api/v1/accounts/{id}/deposits", savingsId)
                        .header("Authorization", bearer(tellerToken))
    ...
    void replayedDepositDoesNotPostAgain() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/accounts/{id}/balance", savingsId)
                        .header("Authorization", bearer(tellerToken)))
                .andExpect(status().isOk())
    ...
    void withdrawalBeyondBalanceIsRefused() throws Exception {
        mockMvc.perform(post("/api/v1/accounts/{id}/withdrawals", savingsId)
                        .header("Authorization", bearer(tellerToken))
    ...
    void overdraftIsHonoured() throws Exception {
        mockMvc.perform(post("/api/v1/accounts/{id}/withdrawals", currentId)
                        .header("Authorization", bearer(tellerToken))
    ...
    void statementIsOrderedAndSigned() throws Exception {
        mockMvc.perform(get("/api/v1/accounts/{id}/transactions", savingsId)
                        .header("Authorization", bearer(tellerToken))
                        .param("size", "10"))
                .andExpect(status().isOk())
    ...
    @Test
    @Order(19)
    @DisplayName("a customer login sees its own accounts and nobody else's")
    void customersSeeOnlyTheirOwnAccounts() throws Exception {
        mockMvc.perform(post("/api/v1/auth/users").header("Authorization", bearer(adminToken))
                        .contentType(JSON).content("""
                        {"username":"asha-it","password":"CustomerPass#2025","roles":["CUSTOMER"],"customerId":"%s"}"""
                        .formatted(customerId)))
                .andExpect(status().isCreated());

        String body = mockMvc.perform(post("/api/v1/auth/login").contentType(JSON).content("""
                        {"username":"asha-it","password":"CustomerPass#2025"}"""))
                .andExpect(status().isOk())
                .andReturn().getResponse().getContentAsString();
        customerToken = JsonPath.read(body, "$.accessToken");

        mockMvc.perform(get("/api/v1/accounts/{id}", savingsId)
                        .header("Authorization", bearer(customerToken)))
                .andExpect(status().isOk());

        // Someone else's account: the general ledger stands in for any account they do not own.
    ...
    void frozenAccountsRejectPostings() throws Exception {
        mockMvc.perform(post("/api/v1/accounts/{id}/freeze", savingsId)
                        .header("Authorization", bearer(tellerToken)))
                .andExpect(status().isOk())
        ...
        mockMvc.perform(post("/api/v1/accounts/{id}/unfreeze", savingsId)
                        .header("Authorization", bearer(tellerToken)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value("ACTIVE"));
    }
    ...
    void fundedAccountsCannotBeClosed() throws Exception {
        mockMvc.perform(post("/api/v1/accounts/{id}/close", savingsId)
                        .header("Authorization", bearer(adminToken)))
                .andExpect(status().isUnprocessableEntity())
    ...
    @Test
    @Order(24)
    @DisplayName("the OpenAPI document is served and describes the bearer scheme")
